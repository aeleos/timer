...
{
	name = "[TIMER] MapChooser Extended",
	author = "Powerlord, Zuko, and AlliedModders LLC, DR. API Improvement",
...
public Action:Command_SetNextmap(client, args)
{
	if (args < 1)
	{
		CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} Usage: sm_setnextmap <map>");
	...
	if (!IsMapValid(map))
	{
		CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Map was not found", map);
		return Plugin_Handled;
	}
...
public Action:Timer_StartMapVote(Handle:timer, Handle:data)
{
	...
	if (timePassed == 0 || !GetConVarBool(g_Cvar_HideTimer))
	{
		...
		switch(timerLocation)
		{
			...
			{
				CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", warningPhrase, warningTimeRemaining);
			}
...
{
	CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Initiated Vote Map");
...
InitiateVote(MapChange:when, Handle:inputlist=INVALID_HANDLE)
{
	...
	if (IsVoteInProgress() || (g_NativeVotes && NativeVotes_IsVoteInProgress()))
	{
		...
		CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Cannot Start Vote", FAILURE_TIMER_LENGTH);
		new Handle:data;
...
//	Call_Finish();
	...
	CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Nextmap Voting Started");
}
						   ...
						   const item_info[][2])
{
	...
	if (strcmp(map, VOTE_EXTEND, false) == 0)
	{
		...
		CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} The current map has been extended. (Received %d%s of %d votes)", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), "%", num_votes);
	...
	else if (strcmp(map, VOTE_DONTCHANGE, false) == 0)
	{
		...
		CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Current Map Stays", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
	...
	else
	{
		...
		CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} Map voting has finished. The next map will be %s. (Received %d%s of %d votes)", map, RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), "%", num_votes);
						   ...
						   const item_info[][2])
{
	...
	if (GetConVarBool(g_Cvar_RunOff) && num_items > 1 && g_RunoffCount < GetConVarInt(g_Cvar_MaxRunOffs))
	{
		...
		if (highest_votes == item_info[1][VOTEINFO_ITEM_VOTES])
		{
			...
			CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Tie Vote", GetArraySize(mapList));
		...
		else if (highest_votes < required_votes)
		{
			...
			CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Revote Is Needed", required_percent);
			SetupWarningTimer(WarningType_Revote, MapChange:g_ChangeTime, mapList);
			return;
		}
	}
...
stock AddMapItem(const String:map[])
{
	...
	else
	{
		...
		Format(display, sizeof(display), "[Tier:%d] %s", Timer_GetMapTier(sMap, TRACK_NORMAL), map);
