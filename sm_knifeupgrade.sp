...
stock Load_Convars()
{
	...
	g_bNoUpdates = GetConVarBool(g_hNoUpdates);
...
stock Hook_Convars()
{
	...
	HookConVarChange(g_hNoUpdates, OnConVarChanged);
	HookConVarChange(g_hHideRestricted, OnConVarChanged);
...
public OnConVarChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	else if (convar == g_hNoUpdates)
		g_bNoUpdates = StringToInt(newValue) == 1 ? true : false;
...
public Action:Updater_OnPluginChecking() {
	if (g_bNoUpdates) return Plugin_Handled;
...
public Action:Event_Say(client, const String:command[], arg)
{
	if (g_bEnabled)
	{
		...
		if (StrEqual(text, Golden, false))
		{
			...
			//Golden Knife
		decl String:Shadow[32];
		Format(Shadow, sizeof(Shadow), "%t", "Knife Trigger Shadow");
		if (StrEqual(text, Shadow, false))
		{
			if (IsValidClient(client))
			{
				if (HasPluginAccess(client)) {
...
{      
	...
	if (knife_choice[client] > 0) 
	{
		...
		if (!CheckCommandAccess(client, "sm_knifeupgrade_shadow", 0, true) && knife_choice[client] == 11) {
			knife_choice[client] = 0;
			CPrintToChat(client, "%t","No Longer Has Access to Current Knife");
		}
	}
...
SetShadow(client)
{
	...
	if (CheckCommandAccess(client, "sm_knifeupgrade_push", 0, true))
	{
		knife_choice[client] = 11;
		SetClientCookie(client, g_cookieKnife, "11");
		CreateTimer(g_fDelayedEquipTimer, CheckKnife, GetClientSerial(client), TIMER_FLAG_NO_MAPCHANGE);
		if (g_bKnifeChosenMessage)
			CPrintToChat(client, "%t","Shadow Given Message");
	} else {
		CPrintToChat(client, "%t", "Knife Access Denied Message");
	}
}
...
public Action:ShowKnifeMenu(client)
{
	...
	Format(Shadow, sizeof(Shadow), "%t", "Menu Knife Shadow");
	...
	if (CheckCommandAccess(client, "sm_knifeupgrade_bayonet", 0, true)) {
		...
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option2", Bayonet,ITEMDRAW_DISABLED);
	}
	//Gut
	if (CheckCommandAccess(client, "sm_knifeupgrade_gut", 0, true)) {
		...
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option3", Gut,ITEMDRAW_DISABLED);
	}
	//Flip
	if (CheckCommandAccess(client, "sm_knifeupgrade_flip", 0, true)) {
		...
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option4", Flip,ITEMDRAW_DISABLED);
	}
	//M9-Bayonet
	if (CheckCommandAccess(client, "sm_knifeupgrade_m9", 0, true)) {
		...
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option5", M9,ITEMDRAW_DISABLED);
	}
	//Karambit
	if (CheckCommandAccess(client, "sm_knifeupgrade_karambit", 0, true)) {
		...
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option6", Karambit,ITEMDRAW_DISABLED);
	}
	//Huntsman
	if (CheckCommandAccess(client, "sm_knifeupgrade_huntsman", 0, true)) {
		...
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option7", Huntsman,ITEMDRAW_DISABLED);
	}
	//Butterfly
	if (CheckCommandAccess(client, "sm_knifeupgrade_butterfly", 0, true)) {
		...
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option8", Butterfly,ITEMDRAW_DISABLED);
	}
	//Falchion
	if (CheckCommandAccess(client, "sm_knifeupgrade_falchion", 0, true)) {
		AddMenuItem(menu, "option9", Falchion);
	} else {
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option9", Falchion,ITEMDRAW_DISABLED);
	}
	//Golden
	if (CheckCommandAccess(client, "sm_knifeupgrade_golden", 0, true) && g_bEnableGoldenKnife) {
		...
		if(!g_bHideRestricted) 
		{
			if (g_bEnableGoldenKnife)
				AddMenuItem(menu, "option10", Golden,ITEMDRAW_DISABLED);
		}
	}
		//Falchion
	if (CheckCommandAccess(client, "sm_knifeupgrade_shadow", 0, true)) {
		AddMenuItem(menu, "option11", Shadow);
	} else {
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option11", Shadow,ITEMDRAW_DISABLED);
	}
	//Default
...
public ShowKnifeMenuHandler(Handle:menu, MenuAction:action, client, itemNum)
{
	switch(action){
		case MenuAction_Select: 
		{
			...
			//Default
			else if (strcmp(info,"option11") == 0) SetShadow(client);
