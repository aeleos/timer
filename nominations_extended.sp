...
{
	name = "[TIMER] Map Nominations Extended",
	author = "Powerlord and AlliedModders LLC, DR. API Improvement",
...
public Action:Command_Addmap(client, args)
{
	if (args < 1)
	{
		CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} Usage: sm_nominate_addmap <mapname>");
	...
	{
		CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Map was not found", mapname);
		return Plugin_Handled;
	}
	...
	if (result > Nominate_Replaced)
	{
		...
		CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Map Already In Vote", mapname);
	...
	CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Map Inserted", mapname);
...
public Action:Command_Nominate(client, args)
{
	...
	if ((status & MAPSTATUS_DISABLED) == MAPSTATUS_DISABLED)
	{
		...
		{
			CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Can't Nominate Current Map");
		}

		if ((status & MAPSTATUS_EXCLUDE_PREVIOUS) == MAPSTATUS_EXCLUDE_PREVIOUS)
		{
			CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Map in Exclude List");
		}

		if ((status & MAPSTATUS_EXCLUDE_NOMINATED) == MAPSTATUS_EXCLUDE_NOMINATED)
		{
			CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Map Already Nominated");
		}
	...
	CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Map Nominated", name, mapname);
...
public Handler_MapSelectMenu(Handle:menu, MenuAction:action, param1, param2)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			if (result == Nominate_AlreadyInVote)
			{
				CPrintToChat(param1, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Map Already Nominated");
				return 0;
			}
			else if (result == Nominate_VoteFull)
			{
				CPrintToChat(param1, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Max Nominations");
				return 0;
			}
			...
			if (result == Nominate_Replaced)
			{
				CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Map Nomination Changed", name, map);
				return 0;
			}

			CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Map Nominated", name, map);
...
stock bool:IsNominateAllowed(client)
{
	...
	switch(result)
	{
		...
		{
			CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Nextmap Voting Started");
			return false;
		}
		...
		case CanNominate_No_VoteComplete:
		{
			...
			CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Next Map", map);
			return false;
		}

		case CanNominate_No_VoteFull:
		{
			CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Max Nominations");
			return false;
		}
	}
