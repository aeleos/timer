...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	CreateNative("Timer_GetStyleRankFast", Native_GetStyleRankFast);
	CreateNative("Timer_GetStyleTotalRank", Native_GetStyleTotalRank);
	CreateNative("Timer_GetBestRound", Native_GetBestRound);
	CreateNative("Timer_GetBestRoundFast", Native_GetBestRoundFast);
...
CollectCache(track, style, Handle:hndl)
{
	...
	while (SQL_FetchRow(hndl))
	{
		...
		decl String:keyBuffer[64];
		Format(keyBuffer, 64, "%s;%d;%d;%d", nNewCache[Auth], timeRank, style, track);
		SetTrieArray(g_tRecordCache, keyBuffer, nNewCache[0], sizeof(nNewCache), true);
		timeRank++;
	}
...
public Native_GetStyleRankFast(Handle:plugin, numParams)
{
	...
	GetClientAuthId(client, AuthId_Steam2, auth, sizeof(auth));
	new nCache[RecordCache];
	for (new i = 0; i < GetTrieSize(g_tRecordCache); i++)
	{
		decl String:keyBuffer[64];
		Format(keyBuffer, 64, "%s;%d;%d;%d", auth, i, style, track);
		if (GetTrieArray(g_tRecordCache, keyBuffer, nCache[0], sizeof(nCache)))
		{
			return i + 1;
		}
	}
...
public Native_GetBestRoundFast(Handle:plugin, numParams)
{
	new client = GetNativeCell(1);
	new style = GetNativeCell(2);
	new track = GetNativeCell(3);
