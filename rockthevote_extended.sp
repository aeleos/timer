...
{
	name = "[TIMER] Rock The Vote Extended",
...
AttemptRTV(client)
{
	...
	{
		CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "RTV Not Allowed");
		return;
	}
		
	if (!CanMapChooserStartVote())
	{
		CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "RTV Started");
		return;
	}
	
	if (GetClientCount(true) < GetConVarInt(g_Cvar_MinPlayers))
	{
		CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Minimal Players Not Met");
		return;			
	}
	
	if (g_Voted[client])
	{
		CPrintToChat(client, "{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Already Voted", g_Votes, g_VotesNeeded);
		return;
	}	
	...
	CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "RTV Requested", name, g_Votes, g_VotesNeeded);
...
StartRTV()
{
	...
	if (EndOfMapVoteEnabled() && HasEndOfMapVoteFinished())
	{
		...
		{
			CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Changing Maps", map);
...
public Action:Command_ForceRTV(client, args)
{
	...
	CPrintToChatAll("{blue}[{lightgreen}LifesGood{blue}]{olive} %t", "Initiated Vote Map");
