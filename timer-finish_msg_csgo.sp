...
public OnTimerRecord(client, track, style, Float:time, Float:lasttime, currentrank, newrank)
{
	...
	if(g_Settings[MultimodeEnable]) 
		FormatEx(StyleString, sizeof(StyleString), " on {lightred}%s", g_Physics[style][StyleName]);
	...
	*/
	if(ranked)
	{
		if(FirstRecord || NewPersonalRecord)
		{
			...
			{
				if(NewWorldRecord){
				CPrintToChatAll("{purple}***NEW WORLD RECORD***");
				CPrintToChatAll("{blue}***NEW WORLD RECORD***");
				CPrintToChatAll("{olive}***NEW WORLD RECORD***");
				CPrintToChatAll("{green}***NEW WORLD RECORD***");
				CPrintToChatAll("{purple}***NEW WORLD RECORD***");
				CPrintToChatAll("{blue}***NEW WORLD RECORD***");
				CPrintToChatAll("{olive}***NEW WORLD RECORD***");
				CPrintToChatAll("{green}***NEW WORLD RECORD***");
