			...
			if(g_iDisplayMethod & cDisplayChatTag)
			{
				...
				if (CheckCommandAccess(author, "sm_cvar", ADMFLAG_ROOT))
				{
					if (g_iCurrentIndex[author] <= 9)
						Format(name, 2048, "{GREY}[Owner Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 19)
						Format(name, 2048, "{RED}[Owner Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 29)
						Format(name, 2048, "{DARKRED}[Owner Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 39)
						Format(name, 2048, "{PURPLE}[Owner Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
				}
				else if (CheckCommandAccess(author, "sm_csay", ADMFLAG_CUSTOM1)) {
					if (g_iCurrentIndex[author] <= 9)
						Format(name, 2048, "{GREY}[Head Admin Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 19)
						Format(name, 2048, "{RED}[Head Admin Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 29)
						Format(name, 2048, "{DARKRED}[Head Admin Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 39)
						Format(name, 2048, "{PURPLE}[Head Admin Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
				}
				else if (CheckCommandAccess(author, "sm_hsay", ADMFLAG_CUSTOM5)) {
					if (g_iCurrentIndex[author] <= 9)
						Format(name, 2048, "{GREY}[Admin Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 19)
						Format(name, 2048, "{RED}[Admin Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 29)
						Format(name, 2048, "{DARKRED}[Admin Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 39)
						Format(name, 2048, "{PURPLE}[Admin Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
				}
				else if (CheckCommandAccess(author, "sm_resetcvar", ADMFLAG_CUSTOM4)) {
					if (g_iCurrentIndex[author] <= 9)
						Format(name, 2048, "{GREY}[Mod Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 19)
						Format(name, 2048, "{RED}[Mod Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 29)
						Format(name, 2048, "{DARKRED}[Mod Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 39)
						Format(name, 2048, "{PURPLE}[Mod Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
				}
				else if (CheckCommandAccess(author, "sm_say", ADMFLAG_CUSTOM3)) {
					if (g_iCurrentIndex[author] <= 9)
						Format(name, 2048, "{YELLOW}[VIP Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 19)
						Format(name, 2048, "{YELLOW}[VIP Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 29)
						Format(name, 2048, "{YELLOW}[VIP Lv.%d] {NORMAL}%s", g_iCurrentIndex[author], name);
