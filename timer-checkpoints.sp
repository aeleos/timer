...
float ClientBestTime[MAXPLAYERS][MAX_STYLES][50];
//int ClientBestTimeID[MAXPLAYERS][MAX_STYLES][50];
...
void ClearClientTimes(int client, bool full)
{
	for (int style = 0; style < MAX_STYLES; style++)
	{
		for (int level = 0; level < 50; level++)
		{
			ClientBestTime[client][style][level] = 0.0;
			//ClientBestTimeID[client][style][level] = 0;
			if (full)
			{
...
{
	char query[255];
	Format(query, sizeof(query), "SELECT `id`,`level`,`time`,`style` FROM `timercheckpoints` WHERE `steamid` = `%s` AND `map` = `%s`", g_sAuth[client], g_sCurrentMap);
...
public void GetClientTimesCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	while (SQL_FetchRow(hndl))
	{
		...
		ClientBestTime[data][style][level] = SQL_FetchFloat(hndl, 3);
		//ClientBestTimeID[data][style][level] = SQL_FetchInt(hndl, 0);
	}
}
...
public int OnClientStartTouchZoneType(int client, MapZoneType type)
{
	if (type == ZtLevel || type == ZtCheckpoint)
	{
		...
		float BestTime = ClientBestTime[client][style][level];
		if (level != g_iLastZoneId[client] + 1)return;
		if (BestTime < (Time1 - g_fTimeBuffer[client]))
		{
			char TimeBuffer[32];
			Timer_SecondsToTime((Time1 - g_fTimeBuffer[client]) - BestTime, TimeBuffer, 32, 2);
			PrintToChat(client, "+%s", TimeBuffer);
		}
		else
		{
			if (BestTime == 0.0)
			{
				PrintToChat(client, "+00:00.00");
				ClientBestTime[client][style][level] = Time1 - g_fTimeBuffer[client];
				char query[255];
				Format(query, sizeof(query), "INSERT INTO `timercheckpoints` (`steamid`,`name,`level`,`time`,`style`) VALUES (`%s`,`%s`,`%d`,`%f`,`%d`);", g_sAuth[client], name, level, ClientBestTime[client][style][level], style);
				SQL_TQuery(g_hSQL, UpdateTimeCallback, query, client, DBPrio_High);
			}
			else
			{
				char TimeBuffer[32];
				...
				ClientBestTime[client][style][level] = Time1 - g_fTimeBuffer[client];
				char query[255];
				Format(query, sizeof(query), "UPDATE `timercheckpoints` SET `time` = `%f` WHERE `steamid` = `%s` AND `map` = `%s` AND  `level` = `%d` AND `style` = '%d';", ClientBestTime[client][style][level], g_sCurrentMap, g_sAuth, style, level);
				SQL_TQuery(g_hSQL, UpdateTimeCallback, query, client, DBPrio_High);
			}
		}
...
public void UpdateTimeCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	if (hndl == INVALID_HANDLE)
	{
		Timer_LogError("SQL Error onUpdateTimeCallback: %s", error);
		return;
	}
	ClearClientTimes(data, false);
	LoadStageTimes(data);
}
