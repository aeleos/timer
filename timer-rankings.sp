...
#include <autoexec>	//https://github.com/Impact123/AutoExecConfig
...
new String:g_sLoadingScoreTag[2048];
new String:g_sLoadingChatTag[2048];
new String:g_sLoadingChatColor[2048];
...
{
	name        = "[TIMER] Rankings",
	author      = "Panduh (AlliedMods: thetwistedpanda), Zipcore, DR. API Improvements",
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	CreateNative("Timer_GetLevel", Native_GetLevel);
...
public OnPluginStart()
{
	...
	LoadTranslations("drapi/drapi_timer-rankings.phrases");
	...
	g_timerRankLoadedForward = CreateGlobalForward("OnPlayerRankLoaded", ET_Event, Param_Cell, Param_Cell, Param_Cell, Param_Cell);
...
ConnectSQL()
{
	...
	else
	{
		...
		for(new i = 1; i <= MaxClients; i++)
		{
			...
			{
				g_bAuthed[i] = GetClientAuthId(i, AuthId_Steam2, g_sAuth[i], 64);
...
public OnMapStart()
{
	...
	if(!g_iEnabled)
		return;
	CreateTimer(1.0, UpdateRanks, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
	GetCurrentMap(g_sCurrentMap, sizeof(g_sCurrentMap));
...
public Action:UpdateRanks(Handle:timer)
{
...
public OnClientPostAdminCheck(client)
{
	...
	g_bAuthed[client] = GetClientAuthId(client, AuthId_Steam2, g_sAuth[client], 64);
	...
	if(GetTrieValue(g_hTrie_CfgCommands, sBuffer, iIndex))
	{
		switch(iIndex)
		{
			case cChatCookie:
			{
				...
				{
					CPrintToChat(client, "%t", "Phrase_Loading");
...
Command_Rank(client)
{
	...
	if(g_iCurrentPoints[client] < g_iRequiredPoints)
		CPrintToChat(client, "%t", "Phrase_Rank_Not_Enough", g_iRequiredPoints, g_iCurrentPoints[client]);
	else
	{
	...
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			if(g_iPositionMethod == 1 || g_iPositionMethod == 2)
			{
				...
				CPrintToChat(param1, "%t", "Phrase_Info_Rank_Rank", iStart, iEnd, sInfo);
			}
			else
			{
				iPoints = GetArrayCell(g_hArray_CfgPoints, iIndex);
				CPrintToChat(param1, "%t", "Phrase_Info_Rank_Points", iPoints, sInfo);
			}

			if(g_iDisplayMethod > 0)
			{
				decl String:sTag[2048], String:sChat[2048], String:sText[2048];
				...
				if(g_iDisplayMethod & cDisplayScoreTag && !StrEqual(sTag, ""))
					CPrintToChat(param1, "%t", "Phrase_Info_Rank_Display_Tag", sTag);
				if(g_iDisplayMethod & cDisplayChatTag && !StrEqual(sChat, ""))
					CPrintToChat(param1, "%t", "Phrase_Info_Rank_Display_Chat", sChat, param1);
				if(g_iDisplayMethod & cDisplayChatColor && !StrEqual(sText, ""))
					CPrintToChat(param1, "%t", "Phrase_Info_Rank_Display_Text", sText);
				if(g_iDisplayMethod & cDisplayScoreStars)
				{
					new iStars = GetArrayCell(g_hCfgArray_DisplayStars, iIndex);
					if(iStars)
					{
						CPrintToChat(param1, "%t", "Phrase_Info_Rank_Display_Stars", iStars);
					}
				}
			}
			...
			if(g_iPositionMethod == 0 && g_iCurrentPoints[param1] < iPoints)
			{
				...
				CPrintToChat(param1, "%t", "Phrase_Info_Rank_Remaining", iPoints);
			}
...
public Action:OnChatMessage(&author, Handle:recipients, String:name[], String:message[])
{
	...
	if(g_iDisplayMethod < 0)
	{
		...
		if((g_hDisplayCookie != INVALID_HANDLE && !g_bLoadedCookies[author]) || !g_bLoadedSQL[author])
		{
			...
			{
				Format(name, 2048, "%s%s", g_sLoadingChatTag, name);
				TrimString(name);
				CFormat(name, 2048, author);
				CFormat(name, 2048, author);
			}
			
			if(iDisplay & cDisplayChatColor)
			{
				Format(message, 2048, "%s%s", g_sLoadingChatColor, message);
				TrimString(name);
				CFormat(name, 2048, author);
				CFormat(message, 2048, author);
			}
		...
		else if(g_iCurrentIndex[author] != -1)
		{
			if(iDisplay & cDisplayChatTag && g_iClientDisplay[author] & cDisplayChatTag)
			{
				...
				Format(name, 2048, "%s%s", sNameBuffer, name);
				TrimString(name);
				CFormat(name, 2048, author);
				CFormat(name, 2048, author);
			}
	...
	else
	{
		...
		else if(g_iCurrentIndex[author] != -1)
		{
			if(g_iDisplayMethod & cDisplayChatTag)
			{
				...
				if (CheckCommandAccess(author, "sm_cvar", ADMFLAG_ROOT))
				{
					if (g_iCurrentIndex[author] <= 9)
						Format(name, 2048, "{grey}[Owner Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 19)
						Format(name, 2048, "{lightred}[Owner Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 29)
						Format(name, 2048, "{darkred}[Owner Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 39)
						Format(name, 2048, "{orchid}[Owner Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
				}
				else if (CheckCommandAccess(author, "sm_csay", ADMFLAG_CUSTOM1)) {
					if (g_iCurrentIndex[author] <= 9)
						Format(name, 2048, "{grey}[Head Admin Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 19)
						Format(name, 2048, "{lightred}[Head Admin Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 29)
						Format(name, 2048, "{darkred}[Head Admin Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 39)
						Format(name, 2048, "{orchid}[Head Admin Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
				}
				else if (CheckCommandAccess(author, "sm_hsay", ADMFLAG_CUSTOM5)) {
					if (g_iCurrentIndex[author] <= 9)
						Format(name, 2048, "{grey}[Admin Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 19)
						Format(name, 2048, "{lightred}[Admin Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 29)
						Format(name, 2048, "{darkred}[Admin Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 39)
						Format(name, 2048, "{orchid}[Admin Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
				}
				else if (CheckCommandAccess(author, "sm_resetcvar", ADMFLAG_CUSTOM4)) {
					if (g_iCurrentIndex[author] <= 9)
						Format(name, 2048, "{grey}[Mod Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 19)
						Format(name, 2048, "{lightred}[Mod Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 29)
						Format(name, 2048, "{darkred}[Mod Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 39)
						Format(name, 2048, "{orchid}[Mod Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
				}
				else if (CheckCommandAccess(author, "sm_say", ADMFLAG_CUSTOM3)) {
					if (g_iCurrentIndex[author] <= 9)
						Format(name, 2048, "{orange}[VIP Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 19)
						Format(name, 2048, "{orange}[VIP Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
					else if (g_iCurrentIndex[author] <= 29)
						Format(name, 2048, "{orange}[VIP Lv.%d] {purple}%s", g_iCurrentIndex[author], name);
...
UpdateClientTag(client)
{
	...
	else
	{
		...
		{
			decl String:sBuffer[20];
			decl String:sBuffer1[20];
			GetArrayString(g_hCfgArray_DisplayTag, g_iCurrentIndex[client], sBuffer, sizeof(sBuffer));
			if (CheckCommandAccess(client, "sm_cvar", ADMFLAG_ROOT))
			{
				Format(sBuffer1, sizeof(sBuffer1), "[Owner Lv.%d]", g_iCurrentIndex[client]);
				CS_SetClientClanTag(client, sBuffer1);
			}
			else if (CheckCommandAccess(client, "sm_csay", ADMFLAG_CUSTOM1)) {
				Format(sBuffer1, sizeof(sBuffer1), "[H-Admin Lv.%d]", g_iCurrentIndex[client]);
				CS_SetClientClanTag(client, sBuffer1);
			}
			else if (CheckCommandAccess(client, "sm_hsay", ADMFLAG_CUSTOM5)) {
				Format(sBuffer1, sizeof(sBuffer1), "[Admin Lv.%d]", g_iCurrentIndex[client]);
				CS_SetClientClanTag(client, sBuffer1);
			}
			else if (CheckCommandAccess(client, "sm_resetcvar", ADMFLAG_CUSTOM4)) {
				Format(sBuffer1, sizeof(sBuffer1), "[Mod Lv.%d]", g_iCurrentIndex[client]);
				CS_SetClientClanTag(client, sBuffer1);
			}
			else if (CheckCommandAccess(client, "sm_say", ADMFLAG_CUSTOM3)) {
				Format(sBuffer1, sizeof(sBuffer1), "[VIP Lv.%d]", g_iCurrentIndex[client]);
				CS_SetClientClanTag(client, sBuffer1);
			}
			else if (CheckCommandAccess(client, "sm_execcfg", ADMFLAG_CUSTOM6)) {
				Format(sBuffer1, sizeof(sBuffer1), "[Squeaker Lv.%d]", g_iCurrentIndex[client]);
				CS_SetClientClanTag(client, sBuffer1);
			}
			else {
				CS_SetClientClanTag(client, sBuffer);
...
public CallBack_LoadRank(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	new lastrank = g_iCurrentIndex[client];
	...
	Call_PushCell(g_iCurrentIndex[client]);
	Call_PushCell(lastrank);
	Call_Finish();
...
public CallBack_Next(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	if(SQL_GetRowCount(hndl))
	{
		...
		CPrintToChat(client, "%t", "Phrase_Next_None");
}
		...
		case MenuAction_Select:
		{
			...
			switch(StringToInt(sOption))
			{
				case 1:
				{
					...
					{
						CPrintToChat(param1, "%t", "Phrase_Loading");
						return;
					}
				...
				case 3:
				{
					...
					if(g_iCurrentPoints[param1] < g_iRequiredPoints)
						CPrintToChat(param1, "%t", "Phrase_Rank_Not_Enough", g_iRequiredPoints, g_iCurrentPoints[param1]);
					else
					{
						decl String:sQuery[192];
	...
	if(SQL_FetchRow(hndl))
	{
		...
		if(g_bGlobalMessage)
		{
			if(!BaseComm_IsClientGagged(client) && (iTime > g_iLastGlobalMessage[client] + cGlobalCooldown))
			{
				...
				CPrintToChatAll("%t", "Phrase_Rank_Global", client, iCount, g_iTotalPlayers, g_iCurrentPoints[client]);

				return;
			}
		}

		CPrintToChat(client, "%t", "Phrase_Rank_Player", iCount, g_iTotalPlayers, g_iCurrentPoints[client]);
	}
}
...
public Action:Timer_AuthClient(Handle:timer, any:userid)
{
	...
	{
		g_bAuthed[client] = GetClientAuthId(client, AuthId_Steam2, g_sAuth[client], 64);
		//LogMessage("Timer_AuthClient: %s", g_sAuth[client]);
		if(!g_bAuthed[client])
			return Plugin_Continue;
		else
		{
...
{
	return g_iCurrentIndex[GetNativeCell(1)];
}
...
CreateSession(client, points, bool:create)
{
	...
	CPrintToChat(client, "%t", "Session started");
}
